...
class ChartingStateNew extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		#if TOUCH_CONTROLS
		var tipText:FlxText = new FlxText(FlxG.width - 210, FlxG.height - 30, 200, 'Press F for Help', 20);
		#else
	...
	function addFileTab()
	{
		...
		btnY += 20;
		#if TOUCH_CONTROLS
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Preview (C)', openEditorPlayState, btnWid);
		#else
		...
		btnY += 20;
		#if TOUCH_CONTROLS
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Playtest (A)', goToPlayState, btnWid);
		#else

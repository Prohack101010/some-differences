...
class CharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.R && !FlxG.keys.pressed.CONTROL #if TOUCH_CONTROLS || _virtualpad.buttonZ.justPressed #end) FlxG.camera.zoom = 1;
		else if ((FlxG.keys.pressed.E #if TOUCH_CONTROLS || _virtualpad.buttonX.pressed #end) && FlxG.camera.zoom < 3) {
		...
		var moveKeys;
			...
			moveKeys = [
				_virtualpad.buttonLeft.pressed,
				_virtualpad.buttonRight.pressed,
				_virtualpad.buttonUp.pressed,
				_virtualpad.buttonDown.pressed,
		...
		else if(FlxG.keys.justPressed.ESCAPE #if TOUCH_CONTROLS || _virtualpad.buttonB.justPressed #end)
		{

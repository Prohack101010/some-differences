...
class ChartingGridSprite extends FlxSprite
{
	...
	public function new(columns:Int, ?color1:FlxColor = 0xFFE6E6E6, ?color2:FlxColor = 0xFFD8D8D8)
	{
		...
		scale.set(ChartingStateNew.GRID_SIZE, ChartingStateNew.GRID_SIZE);
	...
	override function draw()
	{
		...
		scale.y = ChartingStateNew.GRID_SIZE * Math.min(1, rows);
		...
		{
			y += ChartingStateNew.GRID_SIZE + spacing;
			if(y - camera.scroll.y >= FlxG.height)
				break;

			animation.play((i % 2 == 1) ? 'odd' : 'even', true);
			scale.y = ChartingStateNew.GRID_SIZE * Math.min(1, rows - i);
			offset.y = -0.5 * (scale.y - 1);
			super.draw();
		}
		animation.play('even', true);
	...
	function _drawStripes()
	{
		for (i => column in stripes)
		{
			if(column == 0)
				...
				stripe.x = this.x + ChartingStateNew.GRID_SIZE * column - stripe.width/2;
			stripe.draw();
		}
	}
	...
	{
		height = ((ChartingStateNew.GRID_SIZE + spacing) * rows) - spacing;
		updateStripes();
	}
}
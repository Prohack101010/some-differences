...
class MetaNote extends Note
{
	...
	public function changeNoteData(v:Int)
	{
		...
		this.songData[1] = v;
		this.noteData = v % ChartingStateNew.GRID_COLUMNS_PER_PLAYER;
		this.mustPress = (v < ChartingStateNew.GRID_COLUMNS_PER_PLAYER);
		...
		updateHitbox();
		if(width > height)
			setGraphicSize(ChartingStateNew.GRID_SIZE);
		else
			setGraphicSize(0, ChartingStateNew.GRID_SIZE);
	...
	public function setSustainLength(v:Float, stepCrochet:Float, zoom:Float = 1)
	{
		...
		if(sustainLength > 0)
		{
			...
			sustainSprite.setGraphicSize(8, Math.max(ChartingStateNew.GRID_SIZE/4, (Math.round((v * ChartingStateNew.GRID_SIZE + ChartingStateNew.GRID_SIZE) / stepCrochet) * zoom) - ChartingStateNew.GRID_SIZE/2));
	...
	public function findNoteTypeText(num:Int)
	{
		...
		if(num != 0)
		{
			...
			{
				txt = new FlxText(0, 0, ChartingStateNew.GRID_SIZE, (num > 0) ? Std.string(num) : '?', 16);
				txt.autoSize = false;
